...
@section('body')
    <div class="box_btn_search">
        <div class="flex_btn_search">
            ...
            <a class="reload_search" href="{{ route('LayDsSanPham') }}"><ion-icon name="reload-circle-outline"></ion-icon></a>
        </div>
                ...
                @foreach($products as $k => $v) 
                    <tr>
                        ...
                        <td style="width: 200px;" class="text-center">
                            <a href="{{ route('loadCapNhatSanPham',['id' => $v['id']]) }}">
                                @if($v['hinh_anh'] != NULL) 
                                    <img class="img_main" src="{{ asset('upload/sanpham/'.$v['hinh_anh']) }}" width="100" height="100" alt="">
                                @else
                        ...
                        <td class="text-center" style="width: 175px;">
                            <select class="form-select" aria-label="Default select example" name="status_product_ajax_2" id="status_product_ajax_2" data-id="{{ $v['id'] }}"> 
                                @if($v['hang_moi_ve'] == 1)
                                    ...
                                    <option value="2">Không Hàng mới về</option>
                                @elseif($v['hang_moi_ve'] == 2)
                                    <option value="0">Chọn trạng thái</option>
                                    <option value="1">Hàng mới về</option>
                                    <option selected value="2">Không Hàng mới về</option>
                                @else
                                    <option selected value="0">Chọn trạng thái</option>
                                    <option value="1">Hàng mới về</option>
                                    <option value="2">Không Hàng mới về</option>
