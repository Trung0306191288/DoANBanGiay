                ...
                @foreach (TrangChuController::loadLevel1Cate() as $category)
                    <li>
                        @if (TrangChuController::loadLevel2Cate($category->id) != false)
                            <a class="has-child transition" title="{{ $category->ten }}">
...
@foreach (TrangChuController::loadLevel1Cate() as $category)
    <li>
        <a class="has-child transition" href="{{ route('categoriesList',['name_list'=>$category->name,'id_list'=>$category->id]) }}" title="{{ $category->name }}">
